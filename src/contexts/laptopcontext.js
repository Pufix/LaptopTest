...
export function LaptopProvider({ children }) {
    ...
    useEffect(() =>{
        const fetchManufacturers = async () => {
            const response = await fetch('https://servicempp-43ldisly2q-lm.a.run.app/manufacturers');
            if (!response.ok) {
        ...
        const fetchData = async () => {
            const response = await fetch('https://servicempp-43ldisly2q-lm.a.run.app/laptops');
            if (!response.ok) {
                setServerResponse(false);
                console.log("FAILED TO FETCH");
                throw new Error('Failed to fetch data');
        ...
        const testServer = async () => {
            ...
            const response = await fetch('https://servicempp-43ldisly2q-lm.a.run.app/test')
                .then(() => {
